...
{
    [Route("api/ShipRequest")]
    [ApiController]
    [Description("Yêu cầu giao")]
    [Authorize]
    public class ShipRequestController : BaseController<ShipRequest, ShipRequestModel, ShipRequestRequest, ShipRequestSearch>
    {
        protected readonly IShipRequestService ShipRequestService;
        private readonly IConfiguration configuration;

        public ShipRequestController(IServiceProvider serviceProvider, ILogger<BaseController<ShipRequest, ShipRequestModel, ShipRequestRequest, ShipRequestSearch>> logger, IWebHostEnvironment env, IConfiguration configuration) : base(serviceProvider, logger, env)
        {
            this.domainService = this.serviceProvider.GetRequiredService<IShipRequestService>();
            ShipRequestService = serviceProvider.GetRequiredService<IShipRequestService>();
        ...
        public override async Task<AppDomainResult> AddItem([FromBody] ShipRequestRequest itemModel)
        {
            if (!ModelState.IsValid)
            {
                throw new AppException(ModelState.GetErrorMessage());
            }
            var item = mapper.Map<ShipRequest>(itemModel);
        ...
        public virtual async Task<AppDomainResult> ExportExcel([FromQuery] ShipRequestSearch baseSearch)
        {
            string fileResultPath = string.Empty;
            PagedList<ShipRequestModel> pagedListModel = new PagedList<ShipRequestModel>();
            // ------------------------------------------LẤY THÔNG TIN XUẤT EXCEL

            // 1. LẤY THÔNG TIN DATA VÀ ĐỔ DATA VÀO TEMPLATE
            PagedList<ShipRequest> pagedData = await this.domainService.GetPagedListData(baseSearch);
            pagedListModel = mapper.Map<PagedList<ShipRequestModel>>(pagedData);
            ExcelUtilities excelUtility = new ExcelUtilities();

            // 2. LẤY THÔNG TIN FILE TEMPLATE ĐỂ EXPORT
            string getTemplateFilePath = GetTemplateFilePath("ShipRequestTemplate.xlsx");
            ...
            if (pagedListModel.Items == null || !pagedListModel.Items.Any())
                pagedListModel.Items.Add(new ShipRequestModel());
            ...
            string fileName = string.Format("{0}-{1}.xlsx", Guid.NewGuid().ToString(), "ShipRequest");
        ...
        /// <param name="baseSearch"></param>
        /// <returns></returns>
        protected virtual async Task<IDictionary<string, object>> GetParameterReport(PagedList<ShipRequestModel> pagedList, ShipRequestSearch baseSearch)
        {
